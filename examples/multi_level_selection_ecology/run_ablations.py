...
class Boid_world():
    ...
    def get_child_J(parent_agent, selector_network, key):
        ...
        '''
        # just add noise to paret J and return as child J for now, we can add the selector network modulation back in later
        key, subkey = random.split(key)
        J_noise = jax.random.uniform(subkey, shape=parent_J.shape, minval=-SELECTOR_NOISE_SCALE, maxval=SELECTOR_NOISE_SCALE)
        child_J = parent_J + J_noise
        child_J = jnp.clip(child_J, -J_LIMIT, J_LIMIT)
        return child_J
    ...
    def step_world(boid_world, _t):
        ...
        return boid_world.replace(boid_set = boid_set, key=key), (total_age, boid_set.num_active_agents-NUM_ACTIVE_AGENTS, render_data)#, render_data) # return the updated world and the fitness components for logging purposes (total age and number of active agents above the initial number)
