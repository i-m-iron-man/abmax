...
MAX_SPAWN_X = 400.0 # max x in spawning arena
MAX_SPAWN_Y = MAX_SPAWN_X # max y in spawning arena

Dt = 0.1
seeds = [11,7,5]
i = 0 # 1,2 for different seeds
KEY = random.PRNGKey(seeds[i])

DAMPING = 0.1
NOISE_SCALE = 0.05
DATA_PATH = f"./params/seed_{seeds[i]}/"
...
class Patch(Agent):
    ...
    def create_agent(type, params, id, active_state, key):
        ...
        energy = random.uniform(subkeys[2], shape=(1,), minval=10e-4, maxval=energy_max)
...
def main():
    ...
    for generation in range(NUM_GENERATIONS):
        ...
        if mean_fitness > fitness_thresh_save or generation == NUM_GENERATIONS - 1 or generation == 0:
    ...
    jnp.save(DATA_PATH + 'saved_generation_list.npy', saved_generation_list)
